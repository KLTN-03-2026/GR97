...
// Debug route: list users (only for development)
...
// Debug route: reset a user's password (development only)
router.post("/_debug/reset-password", async (req, res) => {
  try {
    ...
    if (useMemoryAuth) {
      ...
      if (!user) return res.status(404).json({ message: "User not found" });
      // memory store uses passwordHash
      user.passwordHash = await bcrypt.hash(newPassword, 10);
      return res.json({ message: "Password updated (in-memory)" });
    }
    ...
    // Setting user.password will trigger pre-save hook to hash
    user.password = newPassword;
    await user.save();
    return res.json({ message: "Password updated" });
  } catch (error) {
    console.error("_debug/reset-password error:", error);
