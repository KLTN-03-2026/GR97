// Replace entire file with a clean, correct implementation
...
const seedMemoryUsers = () => {
  const defaults = [
    // By default only seed a customer user. Admin seeding is gated by SEED_ADMIN=1
    {
  ...
  // Optionally include an admin user when SEED_ADMIN=1 (explicit opt-in)
...
// For debugging: return all users (safe shape)
export const getAllUsers = () => users.map(mapUserResponse);
