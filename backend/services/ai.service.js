...
const formatReadableAdvice = (text) => {
...
const toFriendlyChat = (text) => {
  ...
  return ensurePunctuation(formatReadableAdvice(value));
};
...
const extractRecentUserContext = (history = []) => {
  return history
...
const systemPromptChatty = [
  ...
  "BẮT BUỘC đọc và kết nối các câu hỏi trước đó của người dùng để trả lời theo ngữ cảnh hội thoại, không tách rời từng câu.",
  ...
  "SAU KHI đưa ra lời khuyên, BẮT BUỘC gợi ý 2-3 bác sĩ/chuyên khoa phù hợp để bệnh nhân có thể đặt lịch khám ngay.",
...
const responseFormatInstruction =
  "Trinh bay thanh 3-5 doan ngan, moi doan cach nhau bang mot dong trong; khong tra loi bang mot doan van dai. Sau phan cuoi cung, them JSON gom cac goi y bac si/chuyen khoa: {\"suggestions\": [{\"type\": \"doctor\"|\"specialty\", \"name\": \"Ten bac si hoac chuyen khoa\", \"specialty\": \"Chuyen khoa neu la bac si\", \"reason\": \"Ly do goi y\"}]}.";
...
export const generateSupportReply = async (history, userMessage, options = {}) => {
  ...
  const contextSummary = extractRecentUserContext(history);

  const messages = [
    {
      role: "system",
      content: `${mood === AI_MOODS.PROFESSIONAL ? systemPromptProfessional : systemPromptChatty} ${responseFormatInstruction}`,
    },
    ...history.map((item) => ({ role: item.role, content: item.content })),
    {
      role: "user",
      content: `Ngữ cảnh các câu hỏi gần đây của người dùng:\n${contextSummary || "Chưa có"}\n\nTin nhắn mới: ${userMessage}\nThông tin tham khảo fallback: ${fallback}`,
    },
  ];
  ...
  try {
    ...
    const rawReply = completion.choices?.[0]?.message?.content?.trim() || fallback;
    const reply = mood === AI_MOODS.PROFESSIONAL ? toProfessionalText(rawReply) : toFriendlyChat(rawReply);

    // Parse suggestions from the reply
    let suggestions = [];
    try {
      ...
      // Ignore parsing errors, suggestions will be empty
    }

    return {
      content: reply.replace(/\{[\s\S]*\}/, '').trim(), // Remove JSON from content
      suggestions
    };
  } catch (error) {
