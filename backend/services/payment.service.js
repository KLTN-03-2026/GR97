...
const buildHealthyAIQR = (amount, bookingCode) => {
  const amountStr = Math.round(amount).toLocaleString("vi-VN");
  const payload = `Thanh toán qua hệ thống Healthy AI\nMã đặt lịch: ${bookingCode}\nSố tiền: ${amountStr} VND\nVui lòng thanh toán tại quầy hoặc qua ứng dụng.`;
  return payload;
};
...
export const buildPaymentInfo = ({ amount, bookingCode, orderInfo, ipAddr }) => {
  ...
  const fallbackPayload = buildHealthyAIQR(paymentAmount, bookingCode);
  const fallbackQr = `https://qrcode.tec-it.com/API/QRCode?data=${encodeURIComponent(fallbackPayload)}&size=300`;
  ...
  if (!tmnCode || !hashSecret) {
    return {
      ...
      note: "Chưa cấu hình VNPAY_TMN_CODE/VNPAY_HASH_SECRET. Đang dùng QR VietQR mẫu.",
    };
  }
  ...
  const qrPayload = buildHealthyAIQR(paymentAmount, bookingCode);
  const qrImageUrl = `https://qrcode.tec-it.com/API/QRCode?data=${encodeURIComponent(qrPayload)}&size=300`;
  ...
  return {
    ...
    paymentUrl,
    qrImageUrl,
    note: "Quét QR VietQR để thanh toán qua ứng dụng ngân hàng.",
  };
};
