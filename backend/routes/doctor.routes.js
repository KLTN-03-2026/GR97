...
router.get("/hospitals", cacheMiddleware(600), async (req, res, next) => { // Cache for 10 minutes
  try {
    if (useMemoryAuth) {
...
router.get("/", cacheMiddleware(300), async (req, res, next) => { // Cache for 5 minutes
  try {
    if (useMemoryAuth) {
  ...
  async (req, res, next) => {
    try {
      ...
      if (Number.isNaN(date.getTime())) {
        return res.status(400).json({ message: "appointmentAt is invalid" });
      }

      const slotTime = parseSlotTime(slotLabel);
      if (!slotTime) {
        return res.status(400).json({ message: "slotLabel is invalid" });
      }

      if (date.getHours() !== slotTime.hour || date.getMinutes() !== slotTime.minute) {
        return res.status(400).json({ message: "appointmentAt must match selected timeslot" });
      }

      const minAllowedAt = new Date(Date.now() + BOOKING_LEAD_MINUTES * 60 * 1000);
      if (date <= minAllowedAt) {
        return res.status(400).json({
          message: "Không thể đặt lịch đã qua hoặc quá sát giờ khám. Vui lòng chọn giờ khác.",
        });
      }
